...
public class ToneDetector extends VisualizableDetector implements WorkerFunction.Parameterizable {
    private static final String TAG = Constants.packageTag("ToneDetector");
    ...
    public static final String INFO_KEY_SAMPLING_FREQ = "Sampling Frequency";
    public static final String INFO_KEY_FRAME_SIZE = "Process Frame Size";
    public static final String INFO_KEY_TOLERANCE = "Tolerance (semitone)";
    public static final String INFO_KEY_HANDLE = "Handle";
    public static final String INFO_KEY_UNIT = "unit";
    public static final String INFO_KEY_TARGETS = "Targets";
    public static final String INFO_KEY_HISTORY = "Detection History";

    private static final String ATTR_TARGETS = Constants.Detectors.ToneDetector.PARAM_TARGET_FREQ;
    private static final String ATTR_PROCESS_FRAME_MILLIS =
            Constants.Detectors.ToneDetector.PARAM_PROCESS_FRAME_MILLIS;
    private static final String ATTR_TOL_DIFF_SEMI =
            Constants.Detectors.ToneDetector.PARAM_TOL_DIFF_SEMI;
    private static final String ATTR_CLEAR_TARGETS =
            Constants.Detectors.ToneDetector.PARAM_CLEAR_TARGETS;
    private static final String ATTR_DUMP_HISTORY =
            Constants.Detectors.ToneDetector.PARAM_DUMP_HISTORY;
    ...
    public void setParameter(String attr, Object value) {
        initParameters();
        if (!isValueAccepted(attr, value)) return;

        switch (attr) {
            case ATTR_TARGETS:
                if (value.toString().startsWith("[") && value.toString().endsWith("]"))
                    PARAM_TARGETS.setValue(value.toString());
                else PARAM_TARGETS.setValue("[" + value.toString() + "]");
                return;
    ...
    private boolean checkTargetString(String s) {
        try {
            JSONArray jsonArray;
            if (s.startsWith("[") && s.endsWith("]")) jsonArray = new JSONArray(s);
            else jsonArray = new JSONArray("[" + s + "]");
            for (int i = 0; i < jsonArray.length(); i++) {
                if (jsonArray.getDouble(i) <= 0) return false;
            }
            return true;
    ...
    @Override
    public <T extends View & DetectionListener> T getVisualizedView(
            Context ctx, String token, DetectorBase detector) {
        if (!(detector instanceof VisualizableDetector)) return null;
    ...
    public <T extends View & DetectionListener & DetectorBindable> void bindDetectorView(T v) {
        if (!(v instanceof ToneDetectorView)) return;
    ...
    private void initParameters() {
        mDumpFilePath =
                new File(Constants.externalDirectory(""), getHandle() + ".txt").getAbsolutePath();

        if (mTargetStorage == null) mTargetStorage = new TargetStorage();
        ...
        if (PARAM_PROCESS_FRAME_MILLIS == null)
            PARAM_PROCESS_FRAME_MILLIS =
                    new WorkerFunction.Parameter<>(
                            ATTR_PROCESS_FRAME_MILLIS,
                            false,
                            Constants.Detectors.ToneDetector.Config.PROCESS_FRAME_MILLIS);
        ...
        if (PARAM_TOL_DIFF_SEMI == null)
            PARAM_TOL_DIFF_SEMI =
                    new WorkerFunction.Parameter<>(
                            ATTR_TOL_DIFF_SEMI,
                            false,
                            Constants.Detectors.ToneDetector.Config.TOL_DIFF_SEMI);
    ...
    private void updateCircularBuffers() {
        ...
        for (int c = 0; c < mNumChannels; c++) {
            ...
            for (int i = 0; i < numSamples; i++) buffer.add(0.);
    ...
    public Target getTargetById(int id) {
        if (id < mTargetStorage.content.size()) return mTargetStorage.content.get(id);
    ...
    public void feed(List<? extends Double>[] data) {
        if (data.length != mNumChannels) {
            Log.e(
                    TAG,
                    "The number of channels of the feeding data does not match! (feeding "
                            + data.length
                            + " channels while the active recording activity runs with "
                            + mNumChannels
                            + " channels.");
        }
        ...
        for (CircularArray<Double> buffer : mBuffers) {
            ...
            for (int i = 0; i < signal.length; i++) signal[i] = buffer.get(i);
    ...
    public boolean parseParameters(String params) {
        ...
        if (!setDetectorParameters(params)) return false;
    ...
    public String toString() {
        try {
            ...
            synchronized (mTargetStorage.content) {
                for (Target t : mTargetStorage.content) targets.put(t.toJson());
            }
    ...
    public String getInfo() {
        ...
        sb.append("Process Frame Size: ")
                .append(PARAM_PROCESS_FRAME_MILLIS.getValue())
                .append(" ms\n");
        synchronized (mTargetStorage.content) {
            sb.append("Targets: ")
                    .append(mTargetStorage.content.size())
                    .append(" target(s)")
                    .append("\n");
            for (int i = 0; i < mTargetStorage.content.size(); i++)
                sb.append("\t")
                        .append(i)
                        .append(": ")
                        .append(mTargetStorage.content.get(i).toJson())
                        .append("\n");
        }
    ...
    private void processParamsOfTargetFreqs(JSONObject jsonParams) throws JSONException {
        if (!jsonParams.has(Constants.Detectors.ToneDetector.PARAM_TARGET_FREQ)) return;

        PARAM_TARGETS.setValue(
                jsonParams
                        .getJSONArray(Constants.Detectors.ToneDetector.PARAM_TARGET_FREQ)
                        .toString());
    }

    private void processParamsOfProcessFrameMillis(JSONObject jsonParams) throws JSONException {
        if (!jsonParams.has(Constants.Detectors.ToneDetector.PARAM_PROCESS_FRAME_MILLIS)) return;

        PARAM_PROCESS_FRAME_MILLIS.setValue(
                jsonParams.getInt(Constants.Detectors.ToneDetector.PARAM_PROCESS_FRAME_MILLIS));
    }

    private void processParamsOfTolerance(JSONObject jsonParams) throws JSONException {
        if (!jsonParams.has(Constants.Detectors.ToneDetector.PARAM_TOL_DIFF_SEMI)) return;

        PARAM_TOL_DIFF_SEMI.setValue(
                jsonParams.getInt(Constants.Detectors.ToneDetector.PARAM_TOL_DIFF_SEMI));
    }

    private void processParamsOfDumpHistory(JSONObject jsonParams) throws JSONException {
        if (!jsonParams.has(Constants.Detectors.ToneDetector.PARAM_DUMP_HISTORY)) return;

        boolean newValue =
                jsonParams.getBoolean(Constants.Detectors.ToneDetector.PARAM_DUMP_HISTORY);
        if (newValue == PARAM_DUMP_HISTORY.getValue()) return;
            ...
            synchronized (mTargetStorage.content) {
                if (PARAM_DUMP_HISTORY.getValue()) {
                    ...
                    for (int i = 0; i < mTargetStorage.content.size(); i++) {
                        ...
                        history.append(System.currentTimeMillis())
                                .append(": ")
                                .append(t.targetFreq)
                                .append(" inactive")
                                .append("\n");
                    ...
                    try {
                        PrintWriter pw =
                                new PrintWriter(
                                        new FileOutputStream(new File(mDumpFilePath), true));
            ...
            for (int i = 0; i < targetFreqs.length(); i++) {
                ...
                synchronized (mTargetStorage.content) {
                    for (int j = 0; j < mTargetStorage.content.size(); j++) {
                        ...
                        if (dummy.targetFreq == t.targetFreq) break;
                    }
    ...
    private void process(double[] signal) {
        synchronized (mTargetStorage.content) {
            if (mTargetStorage.content.size() == 0) return;
        }
        ...
        for (Target t : mTargetStorage.content) targetFreqs.add((double) t.targetFreq);

        double minNormFactor = Math.sqrt(spectrum.length / 2) / 20.0;
        double stepFreq = (double) mSamplingFreq / spectrum.length;
        PeakDetector.Config config =
                new PeakDetector.Config.Builder()
                        .withMinNormFactor(minNormFactor)
                        .withNumPoints(5)
                        .withStep(1)
                        .withStepFreq(stepFreq)
                        .addTargetFreqs(targetFreqs)
                        .build();

        ArrayList<double[]> features =
                PeakDetector.extractFeature(Arrays.copyOf(spectrum, spectrum.length / 2), config);
        for (int i = 0; i < features.size(); i++) {
            PeakDetector.QuadraticFeature feature =
                    new PeakDetector.QuadraticFeature(features.get(i), config);
            if (featureActive(feature)) {
                double tfreq = targetFreqs.get(i);
                double freq = tfreq - (feature.coeffs[1] / (2 * feature.coeffs[0])) * stepFreq;
                double amp =
                        Math.pow(feature.coeffs[1], 2) / (4 * feature.coeffs[0])
                                + feature.coeffs[2];
        ...
        if (PARAM_DUMP_HISTORY.getValue()) {
            ...
            for (int i = 0; i < mTargetStorage.content.size(); i++) {
                ...
                if (t.state != detected) {
                    ...
                    history.append(System.currentTimeMillis())
                            ...
                            .append(t.targetFreq)
                            .append(" ")
                            .append(detected ? "active" : "inactive")
                            .append("\n");
                }
            }
            ...
            if (history.length() > 0) {
                try {
                    PrintWriter pw =
                            new PrintWriter(new FileOutputStream(new File(mDumpFilePath), true));
                    pw.write(history.toString());
                    pw.close();
    ...
    public static boolean toneDetected(double freq, double tfreq, double diffInSemiTone) {
